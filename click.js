...
let vSound1 = new Audio("./sound/vilagersound1.mp3");
let vSound2 = new Audio("./sound/vilagersound2.mp3");
let vSound3 = new Audio("./sound/vilagersound3.mp3");
let blockSound = new Audio("./sound/blockhit.mp3");
...
block.addEventListener("click", () => {
  ...
  if (clickAmount === 0) {
    ...
    blockAmount +=
      clickAmount + clickAmount * enchantAccess[1] + enchantAccess[0];
  }
  blockAmountLabel.innerHTML = "Blocks: " + blockAmount;
...
pickaxeUpgradeButton.innerHTML =
  "Pickaxe Upgrade: " + multipliers.pickaxeUpgradeValue;
...
function enchantValuesOnClick() {
  ...
  let tempArray;
  return (tempArray = [fortune, rarity]);
}
...
villagerArray.forEach((villager) => {
  ...
  villager.object.addEventListener("click", () => {
    if (villager.cost === "Maxlevel") {
      return;
    }
    if(blockAmount>=villager.cost){
      startAutoclicker(villager.name,AutoclickerDuration);
...
function startAutoclicker(name) {
switch(name){
  case "Blacksmith":
